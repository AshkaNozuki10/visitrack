...
class LocationController extends Controller{
    ...
    public function updateLocationFromFrontend(Request $request)
    {
        ...
        // Try to identify the building based on coordinates
    ...
    {
        $user = $request->user();
        $trackingEnabled = $user->information->location_tracking_enabled ?? false;
    ...
    private function identifyBuilding(float $latitude, float $longitude): string
    {
        ...
        // Check which building the point is in
        foreach ($buildings as $name => $polygon) {
            if ($this->pointInPolygon([$latitude, $longitude], $polygon)) {
                return $name;
            }
        }
        
        // If not in any specific building but on campus
        if ($this->pointInPolygon([$latitude, $longitude], $this->getCampusBoundary())) {
